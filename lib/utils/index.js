...
type isNaturalNumberType = {
  value: boolean,
  message?: Function
}
export const isNaturalNumber = (data: number): isNaturalNumberType  => {
  const type = 'number'
  ...
  return {
    value: false,
    message: (key, field): string => `\`${key}\` on the \`${field}\` cannot be less than zero.`
  }
}

export const getRequiredFields = (resolverInfo: Object) => {
...
export const makeSelfClearingDataloader = (resolveBatchFunction: Function) => {
  const dataloader = new DataLoader((ids: $ReadOnlyArray<string>) => {
    dataloader.clearAll()
